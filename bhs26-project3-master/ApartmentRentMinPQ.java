 ...
 */
public class ApartmentRentMinPQ {
     ...
     */
    public ApartmentRentMinPQ(int maxN, int id) {
        if (maxN < 0) throw new IllegalArgumentException();
        this.maxN = maxN;
        n = 0;
        keys = new Apartment[maxN + 1];    // make this of length maxN??
        pq   = new int[maxN + 1];
     ...
     * @throws NoSuchElementException no key is associated with index {@code i}
     */
    public void decreaseKey(int i, Apartment key) {
        validateIndex(i);
        if (!contains(i)) throw new NoSuchElementException("index is not in the priority queue");
        if (keys[i].rent == key.rent )
     ...
     * @throws NoSuchElementException no key is associated with index {@code i}
     */
    public void increaseKey(int i, Apartment key) {
        validateIndex(i);
        if (!contains(i)) throw new NoSuchElementException("index is not in the priority queue");
        if (keys[i].rent == key.rent)
    ...
    ***************************************************************************/
    private boolean greater(int i, int j) {
        ...
        return keys[pq[i]].rent > (keys[pq[j]]).rent;
    }
